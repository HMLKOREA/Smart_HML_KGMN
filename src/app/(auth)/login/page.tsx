...
// 로그인 후 랜딩: 키오스크 계정=출하증 대기화면 전용, 운송사=배차관리, 그 외=출하관리
function landingPath(): string {
  const profile = getSession()?.profile;
  if (profile?.is_kiosk) return '/shipping?waiting=1&kiosk=1'; // 키오스크 = 대기화면만
  return profile?.role === 'transporter' ? '/dispatch' : '/shipping';
}
