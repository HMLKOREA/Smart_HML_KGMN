  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {TRANSPORT_CATEGORIES.map(cat => {
        ...
        return (
          <div key={cat.value} className={`bg-gradient-to-r ${gradFrom} ${gradTo} rounded-xl p-5 text-white shadow-lg`}>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                {icon}
                <h3 className="text-lg font-bold">{cat.label}</h3>
              </div>
              <span className="text-sm opacity-75">{dateLabel}</span>
            </div>
            <div className="grid grid-cols-4 gap-3">
              <div>
                <p className="text-xs opacity-70">건수</p>
                <p className="text-xl font-bold">{s.count}<span className="text-xs opacity-60 ml-0.5">건</span></p>
              </div>
              <div>
                <p className="text-xs opacity-70">계획 대수</p>
                <p className="text-xl font-bold">{s.plannedTrucks}<span className="text-xs opacity-60 ml-0.5">대</span></p>
              </div>
              <div>
                <p className="text-xs opacity-70">계획 물량</p>
                <p className="text-xl font-bold">{s.plannedQty.toLocaleString(undefined, { maximumFractionDigits: 1 })}<span className="text-xs opacity-60 ml-0.5">t</span></p>
              </div>
              <div>
                <p className="text-xs opacity-70">달성률</p>
                <p className="text-xl font-bold">{s.rate}<span className="text-xs opacity-60 ml-0.5">%</span></p>
              </div>
            </div>
          </div>
        );
