    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold text-gray-900">
          ...
          <span className="text-base font-normal text-gray-500 ml-2">({userName})</span>
        </h2>
        <p className="text-gray-500 text-sm mt-1">{today} 운송사 현황</p>
      </div>
        ...
        {needsWeighing.length > 0 && (
          <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-semibold text-amber-800">계근 입력 필요: {needsWeighing.length}건</span>
            </div>
            <div className="space-y-1">
              {needsWeighing.map(s => (
                <div key={s.id} className="flex items-center gap-3 text-sm text-amber-700">
                ...
                {todayRequests.length > 0 ? todayRequests.map((s, i) => {
                  ...
                  return (
                    <tr key={i} className={`hover:bg-gray-50/50 ${missingWeight ? 'bg-amber-50/30' : ''}`}>
                      <td className="font-mono text-xs font-medium text-gray-700">{s.shipment_number}</td>
                      <td className="font-medium text-gray-800">{s.customer_name || '-'}</td>
                      <td>{s.product_name || '-'}</td>
                      <td className="text-right tabular-nums">{Number(s.quantity || 0).toLocaleString()}</td>
                      <td className="font-mono text-xs">{s.vehicle_number || '-'}</td>
                      <td>{s.driver_name || '-'}</td>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <p className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent mt-1">
              ...
              <span className="text-sm font-normal text-gray-400 ml-1">ton</span>
            </p>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          {monthly.vehicleAnalysis.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <YAxis type="category" dataKey="vehicle_number" tick={{ fontSize: 11 }} width={100} />
                ...
                <tbody>
                  {monthly.vehicleAnalysis.map((v, i) => (
                    <tr key={i} className="hover:bg-gray-50/50">
                      <td className="font-mono text-sm font-medium text-gray-800">{v.vehicle_number}</td>
